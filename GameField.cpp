...
GameField::GameField(const GameField& other)
	: width(other.width), 
	height(other.height),
	double_attack(other.double_attack) {
	field = new Cell * [height];
	for (int i = 0; i < height; i++) {
		field[i] = new Cell[width];
...
void GameField::addShip(Ship& ship, Coords top_left, Ship::Orientation orientation) {
	if (!this->isShipCoordsOk(top_left, orientation, ship.getSize())) {
		throw ShipPlacementExeption("Ship coordinates out of range");
	}
...
bool GameField::attack(Coords attack_coords) {
	bool flag;
	if (!this->isCoordsInField(attack_coords)) {
		throw AttackOutOfFieldExeption("coords out of field");
	}
	if (field[attack_coords.y][attack_coords.x].ship_is_here) {
		field[attack_coords.y][attack_coords.x].status_cell = Cell::ship;
		flag = field[attack_coords.y][attack_coords.x].ship_segment_pointer->attack();
		if (double_attack) {
			flag = field[attack_coords.y][attack_coords.x].ship_segment_pointer->attack();
			double_attack = false;
		}
...
void GameField::setCellStatus(Coords coords, Cell::status status){
	field[coords.y][coords.x].status_cell = status;
}


Cell::status GameField::getCellStatus(Coords coords){
	return field[coords.y][coords.x].status_cell;
}

bool GameField::isShipHereDestroyed(Coords coords){
	if (field[coords.y][coords.x].ship_is_here) {
		if (field[coords.y][coords.x].ship_segment_pointer->getStatus() == Ship::ShipSegment::destroyed) {
			return true;
		}
	}
	return false;
...
bool GameField::isShipHere(Coords coords){
	...
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
			if (field[y][x].ship_is_here == true) {
				if (field[y][x].ship_segment_pointer->getStatus() == Ship::ShipSegment::damaged) {
					std::cout << "1";
				}
				if (field[y][x].ship_segment_pointer->getStatus() == Ship::ShipSegment::destroyed) {
					std::cout << "0";
				} 
				if (field[y][x].ship_segment_pointer->getStatus() == Ship::ShipSegment::unharmed) {
					std::cout << "2";
				}
			} else {
				std::cout << "o";
...
void GameField::print_field_ver_ox() {
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
			if (field[y][x].status_cell == Cell::empty) {
				std::cout << "x";
			}
			if (field[y][x].status_cell == Cell::ship){
				std::cout << "s";
			}
			if (field[y][x].status_cell == Cell::unknown) {
				std::cout << "o";
			}
		}
		std::cout << "\n";
